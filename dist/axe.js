  ...
  axe._load({
    ...
    checks: [ {
      ...
      evaluate: function(node, options) {
        ...
        var bgNodes = [], bgColor = commons.color.getBackgroundColor(node, bgNodes, useScroll), fgColor = commons.color.getForegroundColor(node, useScroll);
      ...
      id: "only-dlitems",
      evaluate: function(node, options) {
        var child, nodeName, bad = [], children = node.childNodes, hasNonEmptyTextNode = false;
        for (var i = 0; i < children.length; i++) {
          child = children[i];
          nodeName = child.nodeName.toUpperCase();
          if (child.nodeType === 1 && (nodeName !== "DT" && nodeName !== "DD" && nodeName !== "SCRIPT" && nodeName !== "TEMPLATE")) {
            bad.push(child);
          } else {
    ...
    commons: function() {
      ...
      function isInput(element) {
        ...
        return nodeName === "TEXTAREA" || nodeName === "SELECT" || nodeName === "INPUT" && element.type !== "hidden";
      }
      ...
      function shouldCheckAlt(element) {
        var nodeName = element.nodeName.toUpperCase();
        return nodeName === "INPUT" && element.type === "image" || [ "IMG", "APPLET", "AREA" ].indexOf(nodeName) !== -1;
      }
      ...
      text.accessibleText = function(element) {
        function checkNative(element, inLabelledByContext, inControlContext) {
          var returnText = "", nodeName = element.nodeName.toUpperCase();
